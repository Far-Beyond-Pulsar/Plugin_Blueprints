...
fn dt(s: &str) -> DataType { DataType::from_type_str(s) }
fn exec() -> DataType { dt("execution") }
fn float() -> DataType { dt("f32") }
fn boolean() -> DataType { dt("bool") }
fn integer() -> DataType { dt("i64") }
fn string() -> DataType { dt("String") }
...
fn connection_type_for(dt: &DataType) -> ConnectionType {
    if *dt == exec() { ConnectionType::Execution } else { ConnectionType::Data }
}
...
const EVENT_TITLES:  &[&str] = &["On Begin Play","On Tick","On Overlap","On Damage","On Input","On Destroyed","On Replicated","On Notify","On Timer","On Custom"];
const LOGIC_TITLES:  &[&str] = &["Branch","Sequence","Do Once","For Loop","Gate","Flip Flop","Multi Gate","While Loop","Do N","Select"];
const MATH_TITLES:   &[&str] = &["Add","Multiply","Lerp","Clamp","Normalize","Dot Product","Cross Product","Abs","Floor","Sqrt","Sin","Cos","Power","Log","Map Range"];
const OBJECT_TITLES: &[&str] = &["Set Location","Get Component","Spawn Actor","Apply Damage","Get Class","Set Variable","Cast To","Get Actor","Destroy Actor","Get Owner"];
...
fn build_stress_graph() -> BlueprintGraph {
    ...
    for col in 0..COLS {
        for row in 0..ROWS {
            ...
            let (inputs, outputs, w, h) = if is_reroute {
                ...
                (
                    vec![Pin { id: in_id,  name: "".into(), pin_type: PinType::Input,  data_type: primary_dt.clone() }],
                    vec![Pin { id: out_id, name: "".into(), pin_type: PinType::Output, data_type: primary_dt }],
                    22.0_f32, 22.0_f32,
                )
            } else {
                let mut ins  = Vec::new();
                ...
                if !is_event {
                    ...
                    ins.push(Pin { id: pid, name: "".into(), pin_type: PinType::Input, data_type: exec() });
                }
                ...
                if !is_event {
                    ...
                    ins.push(Pin { id: pid, name: pin_in_label(&nt), pin_type: PinType::Input, data_type: primary_dt.clone() });
                }
                ...
                // Exec output
                {
                    ...
                    outs.push(Pin { id: pid, name: "".into(), pin_type: PinType::Output, data_type: exec() });
                }
                ...
                if nt == NodeType::Logic {
                    ...
                    outs.push(Pin { id: t_pid, name: "True".into(), pin_type: PinType::Output, data_type: exec() });
                    outs.push(Pin { id: f_pid, name: "False".into(), pin_type: PinType::Output, data_type: exec() });
                }
    ...
    for col in 0..COLS {
        ...
        for row in 0..ROWS {
            ...
            if let (Some(ref sp), Some(ref dp)) = (exec_out[col][row].clone(), exec_in[nc][row].clone()) {
                push(&mut connections, &mut cid,
                     &node_ids[col][row], sp,
                     &node_ids[nc][row],  dp,
                     &exec());
            }
            ...
            if let (Some(ref sp), Some(ref dp)) = (data_out[col][row].clone(), data_in_p[nc][row].clone()) {
            ...
            if dc < COLS && row + 1 < ROWS && col % 4 == 1 {
                if let (Some(ref sp), Some(ref dp)) = (data_out[col][row].clone(), data_in_p[dc][row+1].clone()) {
                    push(&mut connections, &mut cid,
                         &node_ids[col][row], sp,
                         &node_ids[dc][row+1], dp,
                         &data_type(row + col + 1));
                }
            }
            ...
            if lc < COLS && col % 5 == 0 && row % 4 == 0 {
                if let (Some(ref sp), Some(ref dp)) = (data_out[col][row].clone(), data_in_p[lc][row].clone()) {
                    push(&mut connections, &mut cid,
                         &node_ids[col][row], sp,
                         &node_ids[lc][row],  dp,
                         &data_type(row + col + 2));
            ...
            if row > 0 && row % 6 == 0 && nc < COLS {
                if let (Some(ref sp), Some(ref dp)) = (data_out[col][row-1].clone(), data_in_p[nc][row].clone()) {
                    push(&mut connections, &mut cid,
                         &node_ids[col][row-1], sp,
                         &node_ids[nc][row],     dp,
                         &data_type(row + col));
                }
            }

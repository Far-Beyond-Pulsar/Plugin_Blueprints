...
impl BpRenderer {
    pub fn new() -> Self {
        Self { grid: None, nodes: None, bezier: None, lines: None, pins: None, text: TextRenderer::new() }
    }
...

        {
            ...
            if !wire_instances.is_empty() {
                if let Some(bs) = &mut self.bezier {
                    ...
                    Self::ensure_buf(device, &mut bs.inst_buf, &mut bs.inst_cap, bytes, wgpu::BufferUsages::VERTEX);
            ...
            if !line_verts.is_empty() {
                if let Some(ls) = &mut self.lines {
                    ...
                    Self::ensure_buf(device, &mut ls.vert_buf, &mut ls.vert_cap, bytes, wgpu::BufferUsages::VERTEX);
            ...
            if !nodes.is_empty() {
                if let Some(ns) = &mut self.nodes {
                    ...
                    Self::ensure_buf(device, &mut ns.inst_buf, &mut ns.inst_cap, bytes, wgpu::BufferUsages::VERTEX);
            ...
            if !pins.is_empty() {
                if let Some(ps) = &mut self.pins {
                    ...
                    Self::ensure_buf(device, &mut ps.inst_buf, &mut ps.inst_cap, bytes, wgpu::BufferUsages::VERTEX);
    ...
    fn create_bezier(device: &wgpu::Device, fmt: wgpu::TextureFormat) -> BezierState {
        ...
        BezierState { pipeline, uni_buf, uni_bg, inst_buf, inst_cap: init_cap }
    }
    ...
    fn create_lines(device: &wgpu::Device, fmt: wgpu::TextureFormat) -> LineState {
        ...
        LineState { pipeline, uni_buf, uni_bg, vert_buf, vert_cap: init_cap }
    }

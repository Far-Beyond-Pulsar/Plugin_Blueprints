...
impl TextRenderer {
    ...
    pub fn queue(
        ...
        if text.is_empty() || size_px < 2.0 { return; }
        ...
        for ch in text.chars() {
            let Some(slot) = self.atlas.glyph(ch, size_px) else { continue };
            if slot.width == 0 {
                cx += slot.advance;
                continue;
            }
    ...
    pub fn flush(
        ...
        let need_atlas_bg = self.atlas_bg.is_none()
            || self.atlas.dirty; // after upload dirty=false but we check before upload
        // Actually check view exists and bg is stale:
        self.rebuild_atlas_bg_if_needed(device);

        if self.pipeline.is_none() {
            self.pipeline  = Some(Self::create_pipeline(device, fmt, uni_bgl, self.atlas_bgl.as_ref().unwrap()));
        }
    ...
    fn rebuild_atlas_bg_if_needed(&mut self, device: &wgpu::Device) {
        if self.atlas_bgl.is_none() {
            self.atlas_bgl = Some(device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
    ...
    pub fn flush_with_external_uni(
        ...
        if self.uni_bgl.is_none() {
            self.uni_bgl = Some(device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
